...
def check_dependencies():
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            logger.warning("⚠️ CUDA is not available. The LLM will run on CPU, which may be slow.")
...
def update_settings(model_name, use_api=False, api_key="", api_url=""):
...
def main():
    ...
    parser.add_argument("--model", default="TinyLlama/TinyLlama-1.1B-Chat-v1.0", 
                        help="Model name to use (default: TinyLlama/TinyLlama-1.1B-Chat-v1.0)")
    parser.add_argument("--use-api", action="store_true", help="Use API instead of local model")
    parser.add_argument("--api-key", default="", help="API key for external LLM service")
    parser.add_argument("--api-url", default="", help="API URL for external LLM service")
    ...
    # Update settings
    if not update_settings(args.model, args.use_api, args.api_key, args.api_url):
        return 1
    
    logger.info("✅ LLM setup completed successfully!")
    logger.info(f"Model: {args.model}")
    logger.info(f"Mode: {'API' if args.use_api else 'Local'}")
